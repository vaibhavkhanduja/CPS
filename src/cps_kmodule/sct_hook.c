...
static int get_sct(void){
	...
	if(!sys_call_table){
		cps_info("syscall_table is NULL, quitting...\n");
		return 0;
	}else{
		cps_info("Found syscall_table addr at 0x%p\n", sys_call_table);
	}
	...
	if(!ia32_sys_call_table){
		...
		cps_info("Found ia32_syscall_table addr at 0x%p\n", ia32_sys_call_table);
	}
#endif
...
| and filled with opcode. Also, the stub will survive cps's unloading.      |
|                                                                             |
...
static void *create_stub(syscall_info_t *iter, void *stub){
	...
	//patch addrs
	ud_patch_addr(bytecode, &cps_state);
...
void hook_calls(void){
	...
	if(!sct_map){
		cps_error("Can't get writable SCT mapping\n");
		goto out;
	}
	...
	if(!ia32_sct_map){
		cps_error("Can't get writable IA32_SCT mapping\n");
		goto out;
	}
#endif
